...
| `captar_anm.py`                     | ANM direitos minerários (CFEM). `fonte_tipo='OFICIAL'` + `motivo_invisivel='royalty_indicador'` no INSERT (fix 09/06: CFEM é pagamento royalty, não obra construção — auto-marcado pra esconder do gargalo) | rest_api        |
