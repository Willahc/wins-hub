...
| `wins_hub-api-1`        | build local (`./app/Dockerfile`)        | 8000 (interno) · 127.0.0.1:8001 (host) | FastAPI (2 workers uvicorn) + init    |
...
- **Init System:** Container `api` roda com `init: true` para gerenciar sinais de encerramento e evitar acúmulo de processos zumbis do Chrome/Playwright.
...
| `captar_anm.py`                     | ANM direitos minerários (CFEM). `fonte_tipo='OFICIAL'` + `motivo_invisivel='royalty_indicador'` no INSERT (fix 09/06: CFEM é pagamento royalty, não obra construção — auto-marcado pra esconder do gargalo). 11/06: baixa o CSV (~80-200MB) uma vez pro /tmp e processa com 1 csv.reader sobre o arquivo (resiliência + parse rápido); gargalo restante é o UPSERT de ~18k linhas. | rest_api        |
...
- **Fase 3 — dedup cross-source (22/06)**: o enforce da Fase 5 usava `checar_dup=False` (só ON CONFLICT id_externo) e deixava passar dup cross-source — mesma operação sob fonte/id_externo diferentes (`bndes_saneamento`/`bndes_saude` = mesma op que `bndes_financiamento`; debêntures/cvm internas, ~57/noite). Fix em `portao.py`: `acha_duplicata(..., exclude_id_externo=)` ignora re-pull da própria fonte (segue p/ ON CONFLICT) e **só mira canônicos `visivel=true`**; `DEFAULT_IDX` ganha `id_externo:0`; `DEDUP_CROSS_FONTES={bndes_financiamento,bndes_saneamento,bndes_saude,debentures_infra,cvm_ipe}` liga `checar_dup` só p/ essas (**não** aneel/ibama — têm agregação por complexo e irmãs de nome idêntico legítimas, ex.: Kuara 120 UFVs); guardrail conta descarte de duplicata como "manteria" (senão dedup legítimo alto dispara fail-open). Enforce também ligado em `captar_debentures_infra.py`. Purga única das 57 dups pré-existentes: OCULTADAS (`motivo_invisivel='dedup_cross_source_fase3_20260622'`, não deletadas — carregavam 82 decisores + 17.552 matches), canônico = mais decisores→mais matches→mais antigo; R$22bi CAPEX fantasma fora.
