...
>
> **Atualização 2026-05-13 (sprint dia 5 / Sessão 2):** **+2 captadores scaffolds.** `captar_dnit` via `gov.br/dnit/pt-br` (subdomínios `servicos.dnit.gov.br` retornam HTTP 000 — DNS/geo-block; portal federal responde): **6 obras inseridas** (Complexo Viário Valparaíso/GO, Ponte Transaraguaia BR-230, conservação rodovias PR, Revitaliza-BR Alagoas, etc). `captar_doe_sp` via `basedosdados` SDK — **BLOQUEADO** (SDK requer GCP project com billing; sem credenciais o scaffold retorna 0 obras e loga warning). `basedosdados==2.0.2` adicionado a requirements.
