#!/usr/bin/env python3
"""T4 V9 — Hunter Domain Search nas obras com domínio sem decisor."""
import os
import re
import time
import json
...
cur = conn.cursor()
cur.execute("""
    SELECT ed.dominio, ed.cnpj, ed.empresa_nome, COUNT(o.id) AS obras
    FROM obras o
...
for idx, (dominio, cnpj, empresa, obra_ct) in enumerate(domains, 1):
    ...
    cur.execute("""
        UPDATE obras SET
            ...
            nivel1_linkedin = %s,
            decisor_status = 'DISCOVERED_V9',
            nivel1_origem_enrichment = COALESCE(nivel1_origem_enrichment,'') || ' +V9_domain_search'
        WHERE cnpj = %s
    ...
    for pri2, _negs, fn2, ln2, pos2, email2, score2, status2, link2 in ranked[:5]:
        cur.execute("""
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,'V9_domain_search')
