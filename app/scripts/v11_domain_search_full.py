#!/usr/bin/env python3
"""V11 full — Hunter Domain Search nas obras OURO/PRATA com domínio sem decisor (sem cap).

Diferenças do v10_domain_search.py:
- MAX_DOMAINS=100 (briefing 16/05 noite v7: user pediu enriquecer TODAS)
- INCLUI gov.br (Hunter tem alguma cobertura em concessionárias estaduais)
- origem='V11_full' nos contatos_alternativos + decisor_status='DISCOVERED_V11'
...
UA = "wins-hub-enrichment/11.0 (+williamvnvn@gmail.com)"
MAX_DOMAINS = 100
SLEEP = 2
...
for idx, (dominio, cnpj, empresa, max_capex, obra_ct) in enumerate(domains, 1):
    ...
    cur.execute("""
        UPDATE obras SET
            ...
            nivel1_linkedin = %s,
            decisor_status = 'DISCOVERED_V11',
            nivel1_origem_enrichment = COALESCE(nivel1_origem_enrichment,'') || ' +V11_full'
        WHERE cnpj = %s
    ...
    for pri2, _negs, fn2, ln2, pos2, email2, score2, status2, link2 in ranked[:5]:
        cur.execute("""
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,'V11_full')
