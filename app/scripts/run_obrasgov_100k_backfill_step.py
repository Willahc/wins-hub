#!/usr/bin/env python3
"""Executa um passo retomavel do backfill ObrasGov 100k."""
...
STATE_DEFAULT = Path("/app/logs/obrasgov_100k_backfill_state.json")
...
def load_state(path: Path, start: date) -> dict:
    ...
    try:
        ...
        data.setdefault("next_start", start.isoformat())
...
def main() -> int:
    ...
    state = load_state(path, args.start)
    if state.get("completed"):
        print(f"Backfill ObrasGov ja completo: {path}")
        return 0
    ...
    if next_start > args.until:
        ...
        print("Backfill ObrasGov completo.")
        return 0
    ...
    cmd = [
        sys.executable,
        "-u",
        "/app/scripts/captar_obrasgov_100k.py",
        "--backfill",
    ...
    print(f"Backfill ObrasGov: {next_start} -> {step_end} | cmd={' '.join(cmd)}", flush=True)
