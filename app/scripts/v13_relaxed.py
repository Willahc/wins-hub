...
for idx, (dominio, cnpj, empresa, max_capex) in enumerate(domains, 1):
    ...
    for e in emails:
        ...
        # V13 relaxed: SEM EXCLUDE; aceita sem nome (vai usar handle do email); score >= 50
        # Mantém só o filter de score
        if not email:
            continue
        if score < 50:
            continue
        # nome fallback = handle do email se faltar first/last
        if not first or not last:
            handle = email.split("@", 1)[0]
            parts = re.split(r"[._-]+", handle)
            if len(parts) >= 2:
                first = first or parts[0].title()
                last = last or " ".join(p.title() for p in parts[1:])
            else:
                first = first or handle.title()
                last = last or "(role-based)"
        pri = priority(pos)  # informativo, não filtra
    ...
    cur.execute("""
        UPDATE obras SET
            ...
            nivel1_linkedin = %s,
            decisor_status = 'DISCOVERED_V13',
            nivel1_origem_enrichment = COALESCE(nivel1_origem_enrichment,'') || ' +V13_relaxed'
        WHERE cnpj = %s
    ...
    for cand in ranked[:5]:
        ...
        cur.execute("""
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,'V13_relaxed')
