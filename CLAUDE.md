...
26. **Hunter `/v2/domain-search?company=X` é FUZZY GLOBAL ruim pra BR** — descoberto 30/05 noite: dos 47 hits Hunter pra 299 NULL sem dominio, ~85% foram lixo de match global homônimo (Bayamón Puerto Rico pra Bálsamo SP, Peque-Colombia pra Veré PR, Soledade RS pra Piedade SP, Playas Ecuador pra Lages SC, Estado de Minas jornal pra Estado de Goiás, etc). Hunter DB priorize US/EU/internacional; SPEs solares micro + prefeituras BR pequenas raramente cobertas. **NÃO usar Hunter pra municípios BR ou empresas locais**; reservar pra empresas com presença internacional (Petrobras, Embraer, Vale). **Auditoria pós-hoc obrigatória se já rodou** — preferir rollback total quando >50% dos hits forem suspeitos. Filtro `dominio LIKE '%.br'` é insuficiente (Hunter retorna .com.br errados também). **Insight de produto correlato**: sem dominio = sem decisor enriquecível = sem produto entregável → obras visíveis sem perspectiva de enrichment devem ser invisibilizadas (motivo `sem_dominio_decisor_DDMMAAAA`) pra evitar inflar painel sem entregar valor.
...
```
OURO: 927 obras | PRATA: 59 | BRONZE: 1.773 | PIPELINE: 546 | NULL: 233 (todas com dominio mapeado)
matches_obra_prestador (cron): ~115k | matches_v2 (standalone): ~632k+
Obras visíveis: 3.538 | Data: 30/05/2026 (−2.079 cleanup dia inteiro, −37%)
Hunter: ~295/2.000 restantes | Reset: 11/06/2026 03:20 UTC
Serper: 2.500 créditos gratuitos (ativos)
Disk VPS: ~82%, 8.8GB free
Backup rclone → GDrive: ativo
Commits hoje: f5ef431→9fd0f4e (20 commits PUSHED origin/main) · 10 backups custom format em /home/william/backups/ultra_brief_20260530/
...
**Track C UFV qtd 4-5 + entes BA 30/05 noite final** (−47): Track C 12 CNPJs UFV/SPE com filtro refinado (capex idêntico/quase OU nomes sequenciais) → 13 canonicas (1 CNPJ span 2 UFs) + **39 obras invisibilizadas** marker `ufv_spe_fragmentada_aneel_30052026` (cumulativo 1.498). Bloco B 2 CNPJs BA com nome único repetido 5x (Secretaria Infra `02931604000187` + Fundo Saúde `05816630000152`) → **8 obras invisibilizadas** marker `licitacao_generica_ente_publico_30052026` (mantém 1 canônica/CNPJ). Backup: `pre_track_c.dump`.

**Enriquecimento massivo NULL 30/05 madrugada (1h+)** — pipeline 6 etapas: (1) `discover_cnpj_dynamic.py` Haiku+Serper descobriu **+64 CNPJs** nas 77 NULL sem CNPJ (hit rate 83%); (2) `populate_dominios --capex-floor=0 --limit=600` Serper SearchChain criou **286 empresa_dominios** novos (beneficia todos tiers); (3) `--holding-fallback` BrasilAPI QSA herança holding (28 resolved global, 0 nas NULL — SPEs sem PJ acessível); (4-5) `--cnpj-in=470 cirúrgico` (holding + dominio próprio) — cobertura 62→**233**/553 (+171); (6) Hunter residual nas 299 restantes — **FALHOU** (aprendizado #26), rollback total 47 hits lixo. **Cleanup final sem_dominio** (−320): `motivo_invisivel='sem_dominio_decisor_30052026'` nas NULL sem perspectiva enrichment — sem dominio = sem decisor = sem produto entregável. Visíveis 3.858 → **3.538** (NULL tier agora 233 todas com dominio mapeado). **20 commits PUSHED origin/main** (`f5ef431..9fd0f4e`). Backups: `pre_q2`, `pre_track_c`, `pre_audit_bh`, `pre_sem_dominio`, etc (10 dumps total). Custo total enrichment ~$5 Serper+Haiku + 38 Hunter calls (zero retorno líquido). CSV obras sem dominio: `/home/william/sem_dominio_30052026.csv`.
