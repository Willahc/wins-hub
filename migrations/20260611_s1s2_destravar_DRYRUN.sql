...
END $$;
SELECT 'S1_promovidas' m, classificacao_computed tier, COUNT(*) n FROM obras WHERE observacoes_validacao LIKE '%auto:s1_noticia_manual_20260611%' GROUP BY 2
UNION ALL SELECT 'S2_promovidas', classificacao_computed, COUNT(*) FROM obras WHERE observacoes_validacao LIKE '%auto:s2_reclass_setor_20260611%' GROUP BY 2
ORDER BY 1,3 DESC;
SELECT 'TOTAL_destravadas_matchaveis' m, COUNT(*) n FROM obras o
  WHERE (observacoes_validacao LIKE '%auto:s1_noticia_manual_20260611%' OR observacoes_validacao LIKE '%auto:s2_reclass_setor_20260611%')
    AND classificacao_computed IN ('OURO','PRATA','BRONZE','PIPELINE') AND setor<>'OUTRO';
ROLLBACK;
