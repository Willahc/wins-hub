-- Migration: 2026-05-22 — recompute_classificacao_obra com PRATA condicional
--
-- Root cause: obras PETROLEO_GAS gigantes (Tupi/Lula R$50bi, Búzios R$384bi,
-- Marlim, Roncador) ficam BRONZE apesar de terem decisor com email SMTP
-- verified, porque confianca_match dos decisores fica em ~40 (pattern
-- detector C2 é conservador com empresas-mãe enormes como Petrobras/Vale).
-- Threshold PRATA atual exige >= 50. Resultado: 21 obras ≥R$1bi PETROLEO_GAS
-- (R$665bi total) presas em BRONZE.
--
-- Fix: adicionar branch "PRATA condicional" ENTRE OURO e PRATA atual:
-- promove pra PRATA quando max_score >= 30 + nivel1_email_smtp_verified=true
-- + EXISTS decisor não-replicado real. Threshold 30 (não 0) evita lixo.
--
-- Safeguards:
-- - threshold 30 (não 0)
-- - excluido_em IS NULL (respeita cleanup Sprint 1 dedup decisores)
-- - hipotese != 'REPLICADO_PROVAVEL_FALSO_POSITIVO' (mantém hardening Sprint 1)
-- - SMTP verified obrigatório (email real confirmado)
-- - NÃO altera branch OURO (continua exigindo >=70)
--
