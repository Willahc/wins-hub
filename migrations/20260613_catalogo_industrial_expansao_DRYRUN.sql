...
-- ANTES
SELECT 'ANTES' AS m, codigo, cnaes FROM categorias_servico
...
-- DEPOIS
SELECT 'DEPOIS' AS m, codigo, cnaes FROM categorias_servico
 WHERE codigo IN ('MONT_IND','MANU_IND','CALD_SOL') ORDER BY codigo;

ROLLBACK;

-- ------------------------------------------------------------
-- Rollback (caso necessario apos COMMIT) — remove so os CNAEs adicionados:
